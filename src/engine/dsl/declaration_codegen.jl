"""Return default validator failure message based on declaration style."""
function validator_default_message(style::Symbol)
    style in (:multi, :pos_rest) ? "Streaming validation failed" : "Argument test failed"
end

"""Emit scalar validator-check code for one parsed argument value."""
function emit_scalar_validator_stmt(
    nm::Symbol,
    vfun_expr,
    vmsg_text::String,
    default_msg::String;
...
"""Emit vector/stream validator-check code for one parsed argument collection."""
function emit_stream_validator_stmt(
    nm::Symbol,
    vfun_expr,
    vmsg_text::String,
    default_msg::String
...
"""Parse one declaration macro node into normalized declaration metadata."""
function parse_decl_pipeline!(ctx::CompileCtx, spec::ArgDeclSpec, node::Expr)
    idx = 3
    ...
    nm, rest = extract_name_and_rest(node, idx, spec.macro_name, "argument name")

    nm in ctx.declared_names && throw(ArgumentError("duplicate argument name: $(nm)"))

    meta = extract_decl_meta!(
        rest;
    ...
    if spec.require_flags
        flags = extract_flags!(meta.remain, spec.macro_name)
        isempty(flags) && throw(ArgumentError("$(spec.macro_name) requires at least one flag"))
        register_flags!(ctx, flags, nm, spec.macro_name)
    else
...
"""Emit codegen for required option declaration."""
function emit_decl_opt_required!(ctx::CompileCtx, d)
    ...
    push!(ctx.option_parse_stmts, quote
        local $(tmp_sym) = $(_gr(:pop_value_once!))(_opt_args, $(flags), $(string(nm)), allow_empty_option_value)
        local _raw = $(tmp_sym)[1]
        local $(provided_sym) = $(tmp_sym)[2]
        isnothing(_raw) && $(_gr(:throw_arg_error))($(_gr(:msg_missing_required_option))($(flags[end])))
        local $(nm)::$(T) = $(_gr(:parse_value))($(T), _raw, $(string(nm)))
    end)
    ...
    if d.vfun !== nothing
        push!(ctx.post_stmts, emit_scalar_validator_stmt(
            nm, d.vfun, d.vmsg, validator_default_message(d.style); skip_nothing=false
        ))
    end
end

"""Emit codegen for optional option declaration (env/default/fallback aware)."""
function emit_decl_opt_optional!(ctx::CompileCtx, d)
    ...
    push!(ctx.option_parse_stmts, quote
        ...
        local $(nm)::$(fieldT) =
            if !isnothing(_cli_raw)
                $(_gr(:parse_value))($(T), _cli_raw, $(string(nm)))
            elseif !isnothing(_env_raw)
                $(_gr(:parse_value))($(T), _env_raw, $(string(nm)))
            elseif $(has_default)
                $(_gr(:convert_default))($(T), $(default_expr), $(string(nm)))
            else
                nothing
            end
    ...
    if d.vfun !== nothing
        push!(ctx.post_stmts, emit_scalar_validator_stmt(
            nm, d.vfun, d.vmsg, validator_default_message(d.style); skip_nothing=true
        ))
    end
end

"""Emit codegen for boolean flag declaration."""
function emit_decl_flag!(ctx::CompileCtx, d)
    ...
    push!(ctx.option_parse_stmts, quote
        ...
        local $(nm)::Bool = $(_gr(:pop_flag!))(_opt_args, $(flags))
...
"""Emit codegen for count-style flag declaration."""
function emit_decl_count!(ctx::CompileCtx, d)
    nm, flags, help, help_name = d.nm, d.flags, d.help, d.help_name
    cnt_sym = Symbol("_cnt_", nm)
    provided_sym = Symbol("_provided_", nm)
    count_stmts = [:( $cnt_sym += $(_gr(:pop_count!))(_opt_args, $(f)) ) for f in flags]
...
"""Emit codegen for multi-value option declaration."""
function emit_decl_multi!(ctx::CompileCtx, d)
    nm, T, flags, help, help_name = d.nm, d.T, d.flags, d.help, d.help_name
    provided_sym = Symbol("_provided_", nm)

    push!(ctx.fields, :($(nm)::Vector{$(T)}))
    push!(ctx.option_parse_stmts, quote
        local _vals = $(_gr(:pop_multi_values!))(_opt_args, $(flags), allow_empty_option_value)
        local $(provided_sym) = !isempty(_vals)
        local $(nm)::Vector{$(T)} = [ $(_gr(:parse_value))($(T), v, $(string(nm))) for v in _vals ]
    end)
    ...
    if d.vfun !== nothing
        push!(ctx.post_stmts, emit_stream_validator_stmt(
            nm, d.vfun, d.vmsg, validator_default_message(d.style)
        ))
    end
end

"""Emit codegen for required positional declaration."""
function emit_decl_pos_required!(ctx::CompileCtx, d)
    ...
    push!(ctx.positional_parse_stmts, quote
        isempty(_args) && $(_gr(:throw_arg_error))($(_gr(:msg_missing_required_positional))($(string(nm))))
        local $(provided_sym) = true
        local $(nm)::$(T) = $(_gr(:parse_value))($(T), popfirst!(_args), $(string(nm)))
    end)
...
"""Emit codegen for optional positional declaration (env/default/fallback aware)."""
function emit_decl_pos_optional!(ctx::CompileCtx, d)
...
"""Emit codegen for rest positional declaration."""
function emit_decl_pos_rest!(ctx::CompileCtx, d)
    nm, T, help, help_name = d.nm, d.T, d.help, d.help_name
    provided_sym = Symbol("_provided_", nm)
    ...
    push!(ctx.positional_parse_stmts, quote
        local $(provided_sym) = !isempty(_args)
        local $(nm)::Vector{$(T)} = [ $(_gr(:parse_value))($(T), x, $(string(nm))) for x in _args ]
        empty!(_args)
    end)
...
const DECL_EMITTERS = Dict{Symbol,Function}(
    :opt_required => emit_decl_opt_required!,
    :opt_optional => emit_decl_opt_optional!,
    :flag         => emit_decl_flag!,
    :count        => emit_decl_count!,
    :multi        => emit_decl_multi!,
    :pos_required => emit_decl_pos_required!,
    :pos_optional => emit_decl_pos_optional!,
    :pos_rest     => emit_decl_pos_rest!,
)

"""Dispatch declaration code emission by declaration style."""
function emit_decl_codegen!(ctx::CompileCtx, d)
    em = get(DECL_EMITTERS, d.style, nothing)
